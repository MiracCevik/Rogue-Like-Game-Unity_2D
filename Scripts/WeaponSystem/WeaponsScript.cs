...
public class WeaponsScript : NetworkBehaviour
{
    ...
    {   
        ...
        if (weaponData != null)
        {
            Debug.Log($"Silah: {weaponData.weaponName}, Hasar: {weaponData.damage}, Hýz: {weaponData.attackSpeed}");
        }
    }
    ...
    public void Attack(int totalDamage)
    {
        ...
        Debug.Log($"{weaponData.weaponName} saldýrýsý yapýldý. Hasar: {weaponData.damage}");
    ...
    {
        Debug.Log($"Saldýrý gerçekleþti! Hasar: {damage}");
        DetectHit(damage);
    }
